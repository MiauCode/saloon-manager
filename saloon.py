"""
Saloon Manager Module - Handles billiards/snooker tables and darts management.

This module contains all the table management functionality including timers,
player tracking, pricing, and session history.
"""
...
SALOON_DATA_FILE = Path("tables.json")
...
class TableWidget(QWidget):
    ...
    def ask_player_info(self):
        ...
        member_spin.setRange(0, 8)
        member_spin.setValue(0)
        ...
        paying_spin.setRange(0, 8)
        paying_spin.setValue(2)
    ...
    def show_settings(self):
        ...
        if result == 1:  # OK button (accept)
            ...
            while parent_window and not hasattr(parent_window, 'delete_table'):
...
class SaloonMainWindow(QWidget):
    def __init__(self, tables: List[Table]):
        ...
        self.scroll_widget.setObjectName("scroll_widget")  # Set object name for styling
    ...
    def save_data(self):
        if save_tables(self.tables):
            QMessageBox.information(self, "Saved", "Saloon data saved successfully.")
        else:
            QMessageBox.critical(self, "Error", "Failed to save saloon data.")


def load_tables() -> List[Table]:
    """Load tables from JSON file or return default tables"""
    if SALOON_DATA_FILE.exists():
        try:
            data = json.loads(SALOON_DATA_FILE.read_text())
...
def save_tables(tables: List[Table]) -> bool:
    """Save tables to JSON file"""
    try:
        data = [t.to_json() for t in tables]
        SALOON_DATA_FILE.write_text(json.dumps(data, indent=2))
        return True
    except Exception:
        return False
